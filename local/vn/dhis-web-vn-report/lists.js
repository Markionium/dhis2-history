...
function insertSelectedValueToList(sourceId, targetId){
	var fromList = document.getElementById(sourceId);
	var targetList = document.getElementById(targetId);
	 ...
	 if ( fromList.selectedIndex == -1 ) {
		...
		for(var i=targetList.length-1;i>targetList.selectedIndex;i--){			
			result.push(targetList.options[i]);
		}
		
		for(i=fromList.length-1; i>=0; i--) {
			if(fromList.options[i].selected){
				fromList.options[i].selected = false;
				fromList.options[i].style.backgroundColor="#FF0000";
				result.push(fromList.options[i]);
				fromList.options[i] = null;
			}
		}	
		
		for(var i=targetList.selectedIndex;i>=0;i--){	
			result.push(targetList.options[i]);
		}
		...
		for(var i=result.length-1;i>=0;i--){			
			targetList.options.add(result[i]);
		}
...
function insertValue( theSel, theText, theValue ) {
