...
function searchDataElementName(){
	...
	var params = 'key=' + getFieldValue('key');
		...
		params += '&dataElementGroupId=' + getFieldValue('dataDictionaryList');
...
function searchDataElementNameReceived(xmlObject){
	...
	for ( var i = 0 ; i < dataElements.length ; i++ )
	{
		...
		var url = '';
		if(  de.getElementsByTagName("url")[0].firstChild != null){
			url = de.getElementsByTagName("url")[0].firstChild.nodeValue;
		}
		// add new row
		var newTR = document.createElement('tr');
		if( i%2 == 0){
		...
		// add new column
		newTR = addOperatorColumns( newTR, id, name, url );
		
		tBody.appendChild(newTR);
	}
}

function addOperatorColumns( rowObject, dataElementId, dataElementName, dataElementURL ){
	if ( dataDictionaryMode == "extended" ){
		// add new column
		var newTD = document.createElement('td');
		newTD.innerHTML = '<a href="showUpdateExtendedDataElementForm.action?id=' + dataElementId + '" '
				+ ' title="'+ i18n_edit +'"><img src="../images/edit.png" alt="' + i18n_edit + '"></a>';
		rowObject.appendChild(newTD);
	}else{
		var newTD = document.createElement('td');
		newTD.innerHTML = '<a href="showUpdateDataElementForm.action?id=' + dataElementId + '" title=' + i18n_edit +'>'
				+ '<img src=../images/edit.png alt=' + i18n_edit + '></a>';
		rowObject.appendChild(newTD);
	}
	
	var newTD1 = document.createElement('td');
		newTD1.innerHTML = '<a href="javascript:translate( "DataElement", ' + dataElementId + ' )"  title=' + i18n_translation_translate + '>' 
				+ '<img src="../images/i18n.png" alt="' + i18n_translation_translate + ' )"></a>';
	rowObject.appendChild(newTD1);
	
	var newTD2 = document.createElement('td');
		newTD2.innerHTML = '<a href="javascript:removeDataElement(' + dataElementId + ',' + dataElementName + ' )" title=' + i18n_remove + '>' 
				+ '<img src="../images/delete.png" alt="'+ i18n_remove + '"></a>';
	rowObject.appendChild(newTD2);
	
	var newTD3 = document.createElement('td');
		newTD3.innerHTML = '<a href="javascript:showDataElementDetails( ' +  dataElementId + ')"  title=' + i18n_show_details + '>'
				+ '<img src="../images/information.png" alt="' + i18n_show_details + '"></a>';
	rowObject.appendChild(newTD3);
	
	if ( forumIntegration ){
		var newTD4 = document.createElement('td');
		newTD4.innerHTML = '<a href="' + dataElementURL + '" target="_blank"  title=' + i18n_forum + '>' 
				+ '<img src="../images/forum.png" alt="' + i18n_forum + ' )"></a>';
		rowObject.appendChild(newTD4);
	}
	return rowObject;
}