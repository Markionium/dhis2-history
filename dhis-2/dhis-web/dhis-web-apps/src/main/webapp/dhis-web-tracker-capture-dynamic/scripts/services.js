...
{uid:'rule126246t',trigger:'tracker_data_changed',program_uid:'WSGAb5XwJ3Y',priority:1,programStage_uid:null,condition:'($moderateanemiacurrentevent === false && $severeanemiacurrentevent === false && ($hemoglobincurrentevent != 0 ||  $hematocritcurrentevent != 0) )',actions:[{uid:'actn126110e',action:'displaytext',location:'sum',content:'No anaemia',data:null},
{uid:'actn127121e',action:'assignvariable',location:null,content:'$noanemiacurrentevent',data:'true'}]},
{uid:'rule128123t',trigger:'tracker_data_changed',program_uid:'WSGAb5XwJ3Y',priority:null,programStage_uid:null,condition:'$hemoglobinlatest >= 11',actions:[{uid:'actn128111e',action:'hidefield',location:null,content:'vANAXwtLwcT',data:null}]},
{uid:'rule129123t',trigger:'tracker_data_changed',program_uid:'WSGAb5XwJ3Y',priority:null,programStage_uid:null,condition:'$hematocritlatest >= 33',actions:[{uid:'actn129111e',action:'hidefield',location:null,content:'X8HbdaoS9LN',data:null}]},
...
{uid:'rule132131t',trigger:'tracker_data_changed',program_uid:'WSGAb5XwJ3Y',priority:null,programStage_uid:null,condition:'$noanemiacurrentevent === false',actions:[{uid:'',action:'hidefield',location:null,content:'Kb6kZzUCJi1',data:null}]},
...
{uid:'var139120vv',name:'noanemiacurrentevent',dataType:'bool',defaultValue:'false',sourceType:'calculated_value',dataElement_uid:'',programStage_uid:null,program_uid:'WSGAb5XwJ3Y'},
...
//                }
            ...
            angular.forEach(userDefinedFields, function(fieldCode) {
                ...
                if(fieldCode.sourceType === "dataelement_newest_event_program_stage"){
                    ...
                    if($scope.dhis2Events && $scope.dhis2Events.length > 1) {
                        for(var i = 1; i < $scope.dhis2Events.length; i++) {
                            //When we reach the current stage, and the current stage is not the oldest stage($scope.dhis2Events[0] is the oldest), 
                            //check if the value we want exists in the previous stage.
                            if($scope.dhis2Events[i].programStage !== $scope.currentEvent.programStage) {
                                //Fetch the value from the previous event.
                                var previousvalue = $scope.dhis2Events[i-1][fieldCode.dataElement_uid];
