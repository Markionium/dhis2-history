...
mapfish.GeoStat.Distribution = OpenLayers.Class({
    ...
    defaultLabelGenerator: function(bin, binIndex, nbBins, maxDec) {
		if (GLOBALS.vars.activePanel == GLOBALS.conf.organisationUnitAssignment) {
    ...
    classify: function(method, nbBins, bounds) {
        var mlt = GLOBALS.vars.activePanel == GLOBALS.conf.thematicMap ?
            choropleth.legend.type : GLOBALS.vars.activePanel == GLOBALS.conf.thematicMap2 ?
                proportionalSymbol.legend.type : GLOBALS.vars.activePanel == GLOBALS.conf.organisationUnitAssignment ?
		...
		if (mlt == GLOBALS.conf.map_legend_type_automatic) {
			if (method == mapfish.GeoStat.Distribution.CLASSIFY_WITH_BOUNDS) {
				var str = GLOBALS.vars.activePanel == GLOBALS.conf.thematicMap ? Ext.getCmp('bounds_tf').getValue() : Ext.getCmp('bounds_tf2').getValue();
			...
			bounds = GLOBALS.vars.activePanel == GLOBALS.conf.thematicMap ? choropleth.bounds : proportionalSymbol.bounds;
			...
			method = GLOBALS.vars.activePanel == GLOBALS.conf.organisationUnitAssignment ? mapfish.GeoStat.Distribution.CLASSIFY_BY_EQUAL_INTERVALS : mapfish.GeoStat.Distribution.CLASSIFY_WITH_BOUNDS;
		}
