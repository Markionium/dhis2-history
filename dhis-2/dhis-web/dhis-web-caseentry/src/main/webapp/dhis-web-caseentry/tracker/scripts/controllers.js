...
.controller('SettingsController',
    ...
    if (angular.isObject($scope.selectedOrgUnit)) {
        ...
        $scope.programs = [];
        ProgramFactory.getMine().then(function(data) {
    ...
    $scope.saveSettings = function() {
        ...
        if (angular.isObject($scope.selectedOrgUnit) && angular.isObject($scope.selecteProgram) ) {
            ...
            ProgramFactory.get( $scope.selecteProgram.id).then(function(program){                
...
.controller('RegistrationController',
    ...
    $scope.saveAndRegisterPregnancy = function() {    
        ...
        PersonFactory.register($scope.person).then(function(data) {
...
.controller('PersonEditController',
    ...
    PersonFactory.get($scope.personUid).then(function(person) {
       $scope.person = person; 
       
       PersonAttributesFactory.getRegistrationAttributes().then(function(data){           
           $scope.registrationAttributes = [];          
           angular.forEach(data.personAttributeTypes, function(personAttribute){
               $scope.registrationAttributes[personAttribute.id] = personAttribute;
           });           
       });       
    });
    ...
    $scope.update = function() {
        ...
        delete $scope.person.registrationAttributes;
        PersonFactory.update($scope.person).then(function(data){           
        });        
    };
...
.controller('EnrollmentController',
    ...
    PersonFactory.get($scope.personUid).then(function(person) {
        ...
        $scope.outerEnrollmentForm.$setPristine();
        $scope.enrollment = '';       
        
        //Get enrollment program and required attributes    
        ProgramFactory.get($scope.selectedProgram.id).then(function(program) {
            ...
            //Check if person has active enrollment
            EnrollmentFactory.getEnrollment($scope.selectedOrgUnit.id, $scope.selectedProgram.id, $scope.personUid, 'ACTIVE').success(function(data){

                if( angular.isObject( data.enrollmentList ) ){
                    $scope.enrollment = data.enrollmentList[0]; //trusing the web api that it allows 
                                                            //only one active instance per person+orgunit+program

                    $scope.program.dateOfIncident = $scope.enrollment.dateOfIncident;
                    $scope.program.dateOfIncident = Date.parse($scope.program.dateOfIncident);
                    $scope.program.dateOfIncident = $filter('date')($scope.program.dateOfIncident, 'yyyy-MM-dd');     
                } 
                else{
                    $scope.program.dateOfIncident = '';
                }
            });
        });
    })    
    ...
    $scope.unenroll = function() {      
        ...
        var modalOptions = {
            ...
            actionButtonText: 'unenroll',
            headerText: 'unenroll',
            bodyText: 'are_you_sure_to_unenroll'
        };
        ...
        ModalService.showModal({}, modalOptions).then(function(result){
            ...
            console.log('Enrollment deleted....', $scope.enrollment );
        });
    };
...
.controller('SearchController',
    ...
    $scope.searchPerson = function(listAll) {
        ...
        if (angular.isObject($scope.selectedOrgUnit) && $scope.selectedOrgUnit.name !== 'not_selected') {
            ...
            if (listAll) {
                PersonFactory.getAll($scope.selectedOrgUnit.id).then(function(data) {
                    $scope.persons = data;                  
                    
                    //Get existing registration attributes - this is to label table heading
