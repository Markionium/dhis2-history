...
{
	var data = jQuery( this_ ).metadata({type:'attr',name:'data'});
...
function saveChoiceCustom( programStageId, dataElementId, selectedOption )
{
	...
	valueSaver.setType(jQuery(selectedOption).metadata({type:"attr",name:"data"}).dataElementType);
	valueSaver.save();
}
...
{
	jQuery(this_).css({"background-color":"#ffffcc"});
	...
	{
		  jQuery(this_).css({"background-color":"#ffcc00"});
...
jQuery(document).ready(function(){
	...
	//					jQuery(this).addClass("focusCell");
						DRAG_DIV.showData(jQuery(childrens[0]).metadata({"type":"attr","name":"data"}));
					}
				},
				function(){
	//				if( currentFocus )
	...
	//				jQuery(this).removeClass("focusCell");
		...
		jQuery("input[name='entryfield'],select[name='entryselect']").each(function(){
			jQuery(this).focus(function(){
				currentFocus = this;
	//			jQuery(this).addClass("focusCell");
				DRAG_DIV.showData(jQuery(this).metadata({"type":"attr","name":"data"}));
			});
	...
	//						jQuery(currentFocus).addClass("focusCell");
							DRAG_DIV.showData(jQuery(currentFocus).metadata({"type":"attr","name":"data"}));
						}
...
DRAG_DIV = {
		init : function()
		{
			...
			dragDiv.css({'left': left+'px', 'top': top+'px'});
			dragDiv.draggable();
		},
...
function doComplete()
{
	...
	jQuery("#entryFormContainer input[name='entryfield'],select[name='entryselect']").each(function(){
		...
		if( jQuery(this).metadata({"type":"attr","name":"data"}).compulsory ){
